...
def train(args):
    ...
    if args.log_wandb:
        ...
        wandb.run.name = f"{args.variant}_freeze{args.freeze_backbone}_lr{args.lr}"
    ...
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, num_workers=4, shuffle=True, collate_fn=collate_fn, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, num_workers=4, shuffle=False, collate_fn=collate_fn, pin_memory=True)
    ...
    if args.freeze_backbone:
        print(f">>> Freezing backbone ({args.variant}). Only training RPN and Heads.")
    ...
    for epoch in range(int(args.epochs)):
        ...
        if should_eval:
            ...
            if current_map > best_map:
                ...
                epochs_no_improve = 0  
                os.makedirs(f"checkpoints/{args.freeze_backbone}/{args.lr}", exist_ok=True)
                torch.save(model.state_dict(), f"checkpoints/{args.freeze_backbone}/{args.lr}/fasterrcnn_{args.variant}_best.pth")
    ...
    torch.save(model.state_dict(), f"checkpoints/{args.freeze_backbone}/{args.lr}/fasterrcnn_{args.variant}_final_epoch.pth")
